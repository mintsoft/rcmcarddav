    ...
    public static function makeAccount(array $accountCfg): Account
    {
        ...
        if ($password == "%b") {
            ...
            if (isset($_SESSION['oauth_token']) && is_array($_SESSION['oauth_token'])) {
                $rcube = rcube::get_instance();
                $acctoken = $rcube->decrypt((string) $_SESSION['password']);
            ...
            $httpOptions = [ 'bearertoken' => $acctoken ];
        } else {
        ...
        if ($accountCfg['ssl_client_password']) {
            $httpOptions['ssl_key'] = array($accountCfg['ssl_client_keyfile'], $accountCfg['ssl_client_password']);
        } else {
